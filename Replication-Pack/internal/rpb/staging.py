...
def apply_test_evidence(
    staging_dir: str | Path,
    test_commands: list[str],
    command_results: list[CommandRunResult],
) -> None:
    """Apply test command/evidence metadata to MANIFEST.json and refresh HASHES."""
    ...
    manifest["commands"].setdefault("build", [])
    manifest["commands"].setdefault("proofs", [])
    manifest["commands"]["tests"] = list(test_commands)

    existing_evidence = manifest.get("evidence", [])
    if not isinstance(existing_evidence, list):
        existing_evidence = []
    retained_evidence = [
        item for item in existing_evidence if not (isinstance(item, dict) and item.get("type") == "tests")
    ]
    retained_evidence.extend(result.to_manifest_evidence() for result in command_results)
    manifest["evidence"] = retained_evidence

    claims = manifest.get("claims")
    if not isinstance(claims, list):
        claims = []
    has_p2 = any(isinstance(claim, dict) and claim.get("profile") == "P2" for claim in claims)
    if not has_p2:
        claims.append(
            {
                "claim_id": "CLAIM-TESTED-001",
                "profile": "P2",
                "label": "Tested: declared test commands exited successfully",
                "properties": [],
            }
        )
