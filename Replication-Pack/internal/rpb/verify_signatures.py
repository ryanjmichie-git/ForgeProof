...
def recheck_tests(pack_dir: str | Path) -> list[str]:
    """Rerun manifest-declared tests and return failure messages."""

    pack = load_pack_directory(pack_dir)
    commands = pack.manifest.get("commands")
    if not isinstance(commands, dict):
        return ["manifest_commands_missing_or_invalid"]

    test_commands = commands.get("tests")
    if not isinstance(test_commands, list):
        return ["manifest_test_commands_missing_or_invalid"]
    if not test_commands:
        return []

    run_commands = [item for item in test_commands if isinstance(item, str) and item.strip()]
    if len(run_commands) != len(test_commands):
        return ["manifest_test_commands_invalid_entry"]

    run_dir = pack.root_dir / "SOURCE" if (pack.root_dir / "SOURCE").exists() else pack.root_dir
    with tempfile.TemporaryDirectory(prefix="rpb-recheck-") as temp_dir:
        evidence_dir = Path(temp_dir) / "EVIDENCE" / "TESTS"
        try:
            run_results = run_test_commands(
                run_commands,
                work_dir=run_dir,
            ...
            run_results = exc.results
            failures = [f"recheck_failed:{item.evidence_id}" for item in run_results if item.exit_code != 0]
            if failures:
                return failures
    ...
    expected = [
        item
        for item in (pack.manifest.get("evidence") if isinstance(pack.manifest.get("evidence"), list) else [])
        if isinstance(item, dict) and item.get("type") == "tests"
    ]
    if expected and len(expected) == len(run_results):
        for expected_item, actual in zip(expected, run_results):
            expected_exit = expected_item.get("exit_code")
            if isinstance(expected_exit, int) and expected_exit != actual.exit_code:
                return [
                    f"recheck_exit_mismatch:{expected_item.get('evidence_id', actual.evidence_id)}"
                ]
